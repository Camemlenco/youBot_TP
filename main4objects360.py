...
class YoubotComplete:
    ...
    def rotate_to_angle(self, target_angle_rad, tolerance=0.05):
        ...
        Lê o eixo X (índice 0) conforme solicitado.
        """
        print(f"--- Realinhando (Eixo X) para {math.degrees(target_angle_rad):.1f}° ---")
        while True:
            # CORREÇÃO: Lê o índice 0 (Eixo X / Alpha)
            orientation = self.sim.getObjectOrientation(self.robot_base, self.sim.handle_world)
            curr_theta = orientation[0] 
            ...
            # Nota: Dependendo da orientação do eixo X, pode ser necessário 
            # inverter o sinal aqui (usar -omega ou omega). 
            # Mantive -omega pois funcionava na lógica anterior, mas fique atento.
            self.set_base_velocity(0, 0, -omega)
    ...
    def go_to_map_position(self, target_x, target_y, tolerance=0.05):
        ...
        while True:
            ...
            # Matriz de rotação (Global -> Local) usando o ângulo X
    ...
    def approach_object_with_laser(self, target_dist, target_ang_deg, timeout=30):
        ...
        if not seen:
            ...
            if not found:
                ...
                # Se falhar na busca, força realinhamento para garantir volta segura
...
def execute_full_cycle(bot):
    ...
    for obj in STORAGE_POSES:
        ...
        # === REALINHAMENTO COM EIXO X ===
        # Garante que o robô olhe para frente antes de sair
